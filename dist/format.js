...
export const formatUrl = (url, collapseLong = true) => {
...
// Debugger.enable().
export const formatInitiator = (initiator) => {
    if (!initiator)
        return 'unknown';
...
const projectHeaders = (headers, keys) => {
    if (!headers)
        return {};
    if (keys.includes(HEADER_WILDCARD))
        return { ...headers };
...
export const toOutputRecord = (r, headerKeys, collapseLongUrls = true) => ({
