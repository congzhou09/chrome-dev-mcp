...
const caseInsensitiveEnum = (values) => z.preprocess((v) => (typeof v === 'string' ? (values.find((x) => x.toLowerCase() === v.toLowerCase()) ?? v) : v), z.enum(values));
...
const toOutputRecord = (r, includeHeaders) => ({
...
export function createServer(getClient, switchToTarget, getCurrentTargetId) {
    ...
    const networkRequests = []; // chronological, circular
    const networkByRequestId = new Map(); // requestId → current hop
    const discardedRequestIds = new Set();
    ...
    let networkRegisteredOnClient = null;
    let networkAttachInFlight = Promise.resolve();
    ...
    const attachNetworkTo = async (client) => {
        resetNetworkCapture();
        ...
        client.Network.on('responseReceived', (event) => {
        ...
        client.Network.on('requestServedFromCache', (event) => {
            const record = networkByRequestId.get(event.requestId);
            if (record)
                record.fromCache = true;
        });
        client.Network.on('loadingFinished', (event) => {
            const record = networkByRequestId.get(event.requestId);
            if (!record)
                return;
        ...
        client.Network.on('loadingFailed', (event) => {
            const record = networkByRequestId.get(event.requestId);
            if (!record)
                return;
            record.state = 'failed';
        ...
        client.Page.on('frameNavigated', (event) => {
            const frame = event.frame;
            if (!frame || frame.parentId)
                return; // subframe — leave the buffer alone
            const keepLoaderId = frame.loaderId;
            for (let i = networkRequests.length - 1; i >= 0; i--) {
                if (networkRequests[i].loaderId === keepLoaderId)
                    continue;
    ...
    server.registerTool('evaluate_at_frame', {
        ...
        if (r.value !== undefined) {
            ...
            const props = (r.preview.properties ?? []).map((p) => `  ${p.name}: ${p.value}`).join(',\n');
