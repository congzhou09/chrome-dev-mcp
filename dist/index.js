...
const queueTransition = (work) => {
    ...
    const run = (async () => {
        ...
        if (predecessor) {
            const settled = await withTimeout(predecessor.catch(() => null), TRANSITION_WAIT_TIMEOUT_MS);
...
async function findActivePageTargetId() {
    ...
    for (const target of pageTargets) {
        ...
        try {
            ...
            const probe = await withTimeout(tempClient.Runtime.evaluate({ expression: 'document.visibilityState === "visible"' }), TARGET_PROBE_TIMEOUT_MS).catch(() => TIMED_OUT);
            if (probe === TIMED_OUT) {
...
async function connectToTarget(targetId) {
    ...
    })(), CONNECT_TIMEOUT_MS).catch(async (e) => {
