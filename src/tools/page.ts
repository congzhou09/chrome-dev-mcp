...
const pngSize = (base64: string): { width: number; height: number } | null => {
...
const intersectViewport = (viewport: Rect, region: Rect): Rect | null => {
...
const captureScaleFactor = (metrics: LayoutMetrics | undefined, reported: unknown): number => {
  ...
  if (cssWidth && deviceWidth && deviceWidth > 0) return deviceWidth / cssWidth;
  ...
  server.registerTool(
    'screenshot',
    {
      ...
      inputSchema: z.object({
        region: z
          ...
          .optional()
          .describe(
            'Capture only this rectangle instead of the whole viewport. CSS pixels, measured from the ' +
        ...
        maxEdge: z
          ...
          .default(-1)
          .describe(
            'Longest side of the returned image, counted in its own pixels rather than CSS pixels, so the ' +
    ...
    async ({ maxEdge, region }) => {
      ...
      if (maxEdge > 0 || region) {
        ...
        const probed = await withTimeout(Promise.all([metrics, ratio]), LAYOUT_METRICS_TIMEOUT_MS).catch(
        ...
        if (!css?.clientWidth || !css.clientHeight) {
          ...
          if (!rect) {
            return toolError(
              `Error: the requested region (${region!.width}x${region!.height} at ${region!.x},${region!.y}) ` +
          ...
          if (region || scale < 1) clip = { ...rect, scale };
        }
      }
      ...
      if (!notes.length) return { content: [image] };
      return { content: [image, { type: 'text' as const, text: notes.join(' ') }] };
    },
  );
