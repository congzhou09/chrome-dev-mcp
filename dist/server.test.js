...
function makeMockClient(evaluate = vi.fn(), captureScreenshot = vi.fn(), debuggerMethods = {}) {
    const debugger_ = {
        on: vi.fn(),
...
async function setupMcpClient(cdpClient) {
...
describe('get_title', () => {
    ...
    it('returns not-connected message when Chrome is unavailable', async () => {
        ...
        const result = await client.callTool({ name: 'get_title', arguments: {} });
        expect(result.content[0].text).toMatch(/Chrome is not connected/);
    });
});
describe('get_url', () => {
...
describe('evaluate_js', () => {
    ...
    it('returns error message when expression throws', async () => {
        ...
        expect(result.content[0].text).toBe('Error: ReferenceError: x is not defined');
    });
    it('returns description when value is undefined (e.g. non-serializable object)', async () => {
        ...
        expect(result.content[0].text).toBe('HTMLDivElement');
    });
...
describe('get_debugger_state', () => {
    ...
    it('returns call stack when paused via CDP event', async () => {
        ...
        const pausedHandler = cdpClient.Debugger.on.mock.calls.find(([event]) => event === 'paused')?.[1];
        pausedHandler?.({
            reason: 'breakpoint',
...
describe('get_inspected_element', () => {
    ...
    it('returns guidance message when window.$0 is not set', async () => {
        ...
        const mcpClient = await setupMcpClient(makeMockClient(evaluate));
        const result = await mcpClient.callTool({ name: 'get_inspected_element', arguments: {} });
        expect(result.content[0].text).toMatch(/window\.\$0 = \$0/);
    });
    it('returns not-connected message when Chrome is unavailable', async () => {
