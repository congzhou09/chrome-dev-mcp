...
export function registerCaptureTools(server: McpServer, session: InspectorSession, capture: NetworkCapture): void {
  server.registerTool(
    'clear_captures',
    {
      description:
        'Discard the buffers this server holds, so a following get_console_logs / get_network_requests shows only what happens next. ' +
    ...
    async ({ targets }) => {
      const cleared: { console?: number; network?: number } = {};
      if (targets.includes('console')) cleared.console = session.clearConsoleLogs();
      if (targets.includes('network')) cleared.network = capture.clearBuffer();
      ...
      return {
        content: [{ type: 'text' as const, text: `Cleared ${summary}.` }],
