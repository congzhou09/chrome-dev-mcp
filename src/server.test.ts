...
describe('evaluate_js', () => {
  ...
  // otherwise escape the handler entirely. Both shapes are real: the message degrades to a
  // bare -32603 "Internal error" once replMode is on, which is the mode this tool ships.
  it.each([
    ['-32000 Execution was terminated', { code: -32000, message: 'Execution was terminated' }],
    ['-32603 Internal error (what replMode actually returns)', { code: -32603, message: 'Internal error' }],
    ])('reports a terminated expression rather than throwing: %s', async (_label, response) => {
